...
#if [ $user != "root" ]; then
#	echo "This script should be invoked with sudo privileges"
#	exit 24
#fi
...
apt-get -y install libpapi-dev >> ${LOGFILE}  2>&1

echo "Installing libpfm4-dev"
apt-get -y install libpfm4-dev >> ${LOGFILE}  2>&1

echo "Installing libpapi-dev"
apt-get -y install libpopt-dev:ppc64el >> ${LOGFILE}  2>&1

echo "Installing libpapi-dev"
apt-get -y install binutils-dev >> ${LOGFILE}  2>&1

echo "Installing libpapi-dev"
apt-get -y install g++ >> ${LOGFILE}  2>&1

echo "Installing libpapi-dev"
apt-get -y install gcc >> ${LOGFILE}  2>&1

echo "Installing libpapi-dev"
apt-get -y install libiberty-dev >> ${LOGFILE}  2>&1

echo "Installing libpapi-dev"
apt-get -y install make >> ${LOGFILE}  2>&1

echo "Switch to oprofile directory"
